...
def print_indeces(my_list):
    ...
    """
    for i, car in enumerate(my_list):
        print i, car
...
def halvesies(number_list):
    ...
    return [float(x)/2 for x in number_list]
...
def mult_numbers(number_list):
    ...
    if number_list == []:
        ...
        return reduce(lambda x, y: 1 *x * y, number_list)
...
def average(number_list):
    ...
    return reduce(lambda x, y: float(x+y), number_list)/len(number_list)
